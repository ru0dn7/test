...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $image = $post['image']; // 기본값으로 기존 이미지 사용
    ...
    // 게시물 업데이트
    $updateStmt = $pdo->prepare("UPDATE pluxity_news SET title = :title, category = :category, content = :content, image = :image WHERE id = :id");
        ...
        <form method="POST" enctype="multipart/form-data" class="write-form">
            <div class="form-group">
                ...
                <select name="category" id="category" required>
                    ...
                    <option value="보도자료" <?= $post['category']==='보도자료'?'selected':'' ?>>보도자료</option>
                    <option value="기업뉴스" <?= $post['category']==='기업뉴스'?'selected':'' ?>>기업뉴스</option>
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="current-thumbnail">
                    <img src="<?= htmlspecialchars($post['image']) ?>" alt="현재 썸네일" style="max-width: 200px; margin-bottom: 10px;">
                </div>
                <?php endif; ?>
            ...
            <div class="form-actions">
                ...
                <a href="view.php?id=<?= $id ?>" class="cancel-btn">취소</a>
            </div>
        </form>
    </main>
    <!-- Footer -->
